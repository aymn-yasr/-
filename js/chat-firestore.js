...
import { createMessageElement, createSystemMessageElement, addWelcomeMessageToChat, activeQuoteData, hideActiveQuoteBubble, updatePrivateButtonNotification } from './chat-ui.js';
...
export async function sendPrivateMessage(senderId, senderName, senderAvatar, receiverId, messageText, quotedData = null) {
    ...
    try {
        ...
        // **الأسطر المعدلة والجديدة:**
        const unreadCounterField = `unreadCount_${receiverId}`;
        await privateChatRef.set({
            senderId: senderId,
            receiverId: receiverId,
            lastMessageTimestamp: serverTimestamp(),
            [unreadCounterField]: firebase.firestore.FieldValue.increment(1) // نزيد العداد للمستلم
        }, { merge: true });
...
export function setupPrivateMessageNotificationListener(currentUserId) {
    ...
    db.collection('privateChats').onSnapshot(snapshot => {
        ...
        snapshot.docs.forEach(doc => {
            const data = doc.data();
            // نتحقق من أن المستخدم الحالي هو المستلم
